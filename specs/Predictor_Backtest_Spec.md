...
- **Collin projection** & **net TARP** — `data/edge/edge2025.json`, **2025 only in THIS run** (the 2022/23 TARP files `tarp2022.json`/`tarp2023.json` are captured per memory but were not in the repo at run time, so not joined here — not fabricated). Collin as a wins prediction; net TARP (a ± adjustment, not wins) tested as correlation with the market residual.
...
3. **Collin and TARP (2025) add nothing over market; TARP corr with the market residual ≈ 0.** *Guardrail:* this 2025-only result is consistent with the **locked multi-year verdict** (memory): TARP net/off/def is weak and sign-unstable across years, and 6-Win% does not replicate (pooled ≈ 0). Multi-year evidence already exists (`tarp2022.json`/`tarp2023.json` held) — **do NOT re-litigate or re-promote** TARP/6-Win% as a beat-market edge. Edge-source role = within-tier refinement only.
