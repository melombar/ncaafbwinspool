...
seq="".join(f'''<tr><td style="text-align:center;color:#8aa0b4">{i+1}</td><td><b>{c}</b></td><td style="text-align:center">R{b['avg_round']}</td>
  <td>{b['top'][0]['team']} <span class="pct">{b['top'][0]['pct']}%</span></td>
  <td class="bk">{', '.join(x['team'] for x in b['top'][1:])}</td>
  <td><span class="tag {'ge' if cliff[c]<=5 else 'df' if cliff[c]>=10 else 'md'}">{'grab early' if cliff[c]<=5 else 'defer OK' if cliff[c]>=10 else 'mid'}</span></td></tr>''' for i,(c,b) in enumerate(order))
...
.pct{{color:#7bd6ff;font-size:11px;font-weight:600}} .bk{{color:#8aa0b4;font-size:12px}}
